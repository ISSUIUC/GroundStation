...
function setup_charts() {
  return {
    lowgimu_accel: make_chart_multiaxis(
      'G',
      'lowgimuA',
      'LowG IMU acceleration',
      [DP_LOWGAX, DP_LOWGAY, DP_LOWGAZ]
    ),
    lowgimu_gyro: make_chart_multiaxis(
      'DPS',
      'lowgimuG',
      'LowG IMU gyroscope',
      [DP_LOWGGX, DP_LOWGGY, DP_LOWGGZ]
    ),
    lowgimu_mag: make_chart_multiaxis(
      'Gauss',
      'lowgimuM',
      'LowG IMU magnetometer',
      [DP_LOWGMX, DP_LOWGMY, DP_LOWGMZ]
    ),
    gps: make_chart('Meters', 'gps', 'GPS altitude', DP_GPS_ALT),
    se: make_chart_multiaxis('Meters', 'se', 'State Estimation', [
      DP_STE_ALT,
      DP_STE_VEL,
      DP_STE_ACC,
      DP_STE_APO
    ]),
    highg_kx_accel: make_chart_multiaxis('G', 'KX134', 'KX acceleration', [
      DP_KXAX,
      DP_KXAY,
      DP_KXAZ
    ]),
    baro_altitude: make_chart(
      'Meters',
      'barometer',
      'Barometer altitude',
      DP_BAROMETER
...
export function run_frontend(
  ...
  serverConnection.on('data', (message) => {
    document.getElementById('packetstats').innerText =
      '0 seconds since last packet'
